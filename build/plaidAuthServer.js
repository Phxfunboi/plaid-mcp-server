...
export function setupPlaidAuthServer(server) {
    // Tool 1: Create a link token for Plaid Link with Auth product
    server.tool("create-auth-link-token", {
    ...
    server.tool("set-update-frequency", {
        userId: z.string().describe("Unique identifier for the user"),
    ...
    server.tool("get-bank-transfer-events", {
        userId: z.string().describe("Unique identifier for the user"),
    ...
    server.prompt("connect-bank-account", {
        userId: z.string().describe("Unique identifier for the user")
    }, ({ userId }) => {
...
async function fetchAndStoreAuthData(userId, accessToken) {
