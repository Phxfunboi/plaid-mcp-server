...
import { Configuration, PlaidApi, Products, CountryCode, PlaidEnvironments, DepositoryAccountSubtype } from 'plaid';
...
export function setupPlaidTransactionsServer(server) {
    // Tool 1: Create a link token for Plaid Link with Auth and Transactions products
    server.tool("create-link-token", {
    ...
    server.tool("exchange-public-token", {
        userId: z.string().describe("Unique identifier for the user"),
    ...
    server.tool("get-recurring-transactions", {
        userId: z.string().describe("Unique identifier for the user"),
    ...
    server.tool("set-refresh-schedule", {
        userId: z.string().describe("Unique identifier for the user"),
    ...
    server.resource("webhook-events", "webhook-events://{userId}", async (uri, extra) => {
...
async function fetchAccounts(userId, accessToken) {
    try {
...
async function syncTransactions(userId, accessToken, keepSyncing = true) {
    ...
    try {
        ...
        if (allRemoved.length > 0) {
            ...
            userData.transactions[userId] = userData.transactions[userId].filter(t => !removedIds.includes(t.transaction_id));
        }
        // Update modified transactions
        if (allModified.length > 0) {
            for (const modifiedTx of allModified) {
                const index = userData.transactions[userId].findIndex(t => t.transaction_id === modifiedTx.transaction_id);
                if (index !== -1) {
...
function scheduleTransactionSync(userId, frequency, customCron) {
